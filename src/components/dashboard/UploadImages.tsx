        ...
        <div className="relative flex flex-col items-center">
            ...
            <div className="mt-6 grid grid-cols-2 gap-4 w-full">
              {uploadImgs.map((img, index) => (
                <div key={index} className="relative group">
                  <img
                    className="w-full h-36 object-cover rounded-lg shadow-lg transition-transform duration-300 group-hover:scale-105"
                    src={img}
                    alt="Uploaded preview"
                  />
                  <button
                    onClick={() => removeImage(index)}
                    className="absolute top-2 right-2 bg-red-600 text-white px-2 py-1 text-xs rounded-full opacity-50 group-hover:opacity-100 transition-opacity duration-200"
                  >
                    ✕
                  </button>
                </div>
          ...
          {uploadImgs.length > 0 && (
            <button
              ...
              className="mt-6 w-full py-3 bg-red-600 text-white rounded-lg font-semibold hover:bg-red-700 transition duration-300"
            >
              Remove All Images
            </button>
          )}
